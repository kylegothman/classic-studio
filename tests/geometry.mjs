...
import {sourcePlate,serializeShells,shellURL} from '../tools/bake-shells.mjs';
...
assert.deepEqual(bakedBytes,serializeShells(cache,sourceModelSha256),'Baked shells are stale; run npm run bake:shells.');
const baked=await new GLTFLoader().parseAsync(bakedBytes.buffer.slice(bakedBytes.byteOffset,bakedBytes.byteOffset+bakedBytes.byteLength),'');
assert.equal(baked.scene.userData.sourceModelSha256,sourceModelSha256);
const bakedCache=readShellCache(baked.scene);
for(const [source,variants]of [['baker',cache.variants],['asset',bakedCache.variants]])for(const [key,v]of variants){
 const g=v.geometry;g.computeBoundingBox();const b=g.boundingBox;close(b.max.z-b.min.z,v.depth,key+' exact depth');close(b.max.x-b.min.x,dims.width+.02,key+' fine rim width');close(b.max.y-b.min.y,dims.height+.02,key+' fine rim height');close(b.min.z,v.rearZ,key+' rear');
 ...
 console.log(source,key,Math.round((g.index?.count??g.attributes.position.count)/3)+' triangles', 'depth '+(b.max.z-b.min.z).toFixed(4));
}
console.log(`PASS: actual GLB outline, ${checked} flat-face triangles, six hollow shells, depth/wall/seam dimensions and real top/bottom apertures (${cache.buildMs} ms build-time CSG; no runtime CSG).`);
