...
Searchable swatches are grouped by finish. A selected card and hover label show the item, vendor, and from price. Factory engraving styles are limited to the chosen back color; capacity text is cosmetic and independent of storage capacity. Details contains dock bezels and depth-specific hold/jack assemblies. Mismatched face/button materials and hold/body depth produce warnings. The hold-depth warning has a one-click fix that preserves black/white where available, or offers black for the thin-only U2 assembly. Saved selections remain intact until that fix is applied.
