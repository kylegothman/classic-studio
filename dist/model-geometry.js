...
export function readShellCache(scene){
 ...
 const variants=new Map();scene.traverse(mesh=>{if(mesh.isMesh)variants.set(mesh.userData.variantKey,{...mesh.userData,geometry:mesh.geometry});});
 if(schemaVersion!==1||!plate||wall!==.02||variants.size!==6||!['thin','thick'].every(body=>['original','usbc','moon'].every(layout=>variants.has(body+':'+layout))))throw new Error('The baked shell asset is incomplete. Regenerate it with npm run bake:shells.');
